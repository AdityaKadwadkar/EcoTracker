...
serve(async (req) => {
  ...
  try {
    ...
    // 1. SAVE ENTRY FIRST (Guarantees data safety)
    ...
    const prompt = `
You are a mental health journaling companion.
User's mood: ${category}.
User's thought: "${entry}"

Respond with:
1) A validation of their feelings.
2) A gentle, actionable piece of advice or incorrect thought pattern challenge.
3) A short encouraging closing.
    ...
    try {
      ...
      // Fallback: Return success but with a placeholder message.
      // The entry is SAVED, which is the most important part.
      return responseJSON({
        feedback: "Your entry is safely saved! The AI is currently taking a moment to breathe (Rate Limit Reached). Please check back on this entry later."
      });
    }
