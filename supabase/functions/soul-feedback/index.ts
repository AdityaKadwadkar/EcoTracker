...
serve(async (req) => {
  ...
  try {
    ...
    const prompt = `
You are a spiritual and emotional wellbeing guide.
Reflect on the user's ${category} entry.

User entry:
"${entry}"

Respond with:
1) A gentle reflection on their emotional/spiritual state.
2) One gratitude or mindfulness practice they can try.
3) One intention or soulful challenge for tomorrow.
    `;
    ...
    try {
      ...
      return responseJSON({
        feedback: "Your reflection is saved. The guidance spirit is quieting its mind (System Busy). Please try again later."
      });
    }
